...
// стили шрифта различных элементов окна (определено в WM_CREATE)
...
int wW = 770; // MIN SIZE - 715
int wH = 570; // MIN SIZE - 480
...
void RedrawWindowElements(int W, int H); // обновляет положение элементов внутри окна при изм. размеров
...
void PrepareSoundGraph(rec_list* Head, ULONG BufferSize, double RecBufferLenght)
{
	...
	{
		RECT rcParent;
		GetWindowRect(hWnd, &rcParent); // получаем размеры и координаты главного окна
		int widthWaitWindow = 250; // ширина нашего мини окна
		int heightWaitWindow = 80; // высота нашего мини окна
...
void RedrawWindowElements(int W, int H)
{
	// раздел записи, КНОПКИ
	SetWindowPos(hBut1, NULL, 60, H-240, 100, 30, SWP_NOSIZE | SWP_NOZORDER);
	SetWindowPos(hBut2, NULL, 180, H-240, 100, 30, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, ЧЕКБОКС
	SetWindowPos(hCheckBoxNoSave, NULL, 15, H-200, 310, 20, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, выбор устройства записи (считывания) СПИСОК
	SetWindowPos(hComboBox5, NULL, 8, H-170, 314, 90, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, глубина звука (бит)
	SetWindowPos(hComboBox1, NULL, 8, H-140, 100, 100, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, частота дискретизации (Гц)
	SetWindowPos(hComboBox2, NULL, 112, H-140, 107, 120, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, МОНО-СТЕРЕО
	SetWindowPos(hComboBox3, NULL, 232, H-140, 100, 70, SWP_NOSIZE | SWP_NOZORDER);
	// раздел записи, РАМКА
	SetWindowPos(hGroupBox2, NULL, 5, H-265, 330, 160, SWP_NOSIZE | SWP_NOZORDER);

	// раздела воспроизведения, КНОПКИ
	SetWindowPos(hBut3, NULL, 10, H - 80, 90, 30, SWP_NOSIZE | SWP_NOZORDER);
	SetWindowPos(hButPPause, NULL, 105, H - 80, 110, 30, SWP_NOSIZE | SWP_NOZORDER);
	SetWindowPos(hBut5, NULL, 238, H - 80, 90, 30, SWP_NOSIZE | SWP_NOZORDER);
	// раздел воспроизведения, РАМКА Y из 340 в H-100
	SetWindowPos(hGroupBox3, NULL, 5, H-105, 330, 65, SWP_NOSIZE | SWP_NOZORDER);

	// список hList1 куда выводим диагностическую информацию вместо коммандной строки
	SetWindowPos(hList1, NULL, 340, H - 190, 350, 155, SWP_NOZORDER); // меняем также размер
	// очистка hList1, КНОПКА
	SetWindowPos(hBut4, NULL, 610, H - 35, 80, 25, SWP_NOSIZE | SWP_NOZORDER);// вкл/выкл визуализации hList1, hBut4
	// вкл/выкл визуализации hList1, hBut4
	SetWindowPos(hCheckBoxShowMessageLog, NULL, 340, H - 35, 230, 25, SWP_NOSIZE | SWP_NOZORDER);

	// раздел обрезки звука, ТРЕКБАРЫ
	SetWindowPos(hTrBar1, NULL, -3, H-320, W +6, 25, SWP_NOZORDER);
	SetWindowPos(hTrBar2, NULL, -3, H-295, W +6, 25, SWP_NOZORDER);
	// раздел обрезки звука, КНОПКА
	SetWindowPos(hButCutSound, NULL, W - 160, H-265, 0, 0, SWP_NOSIZE | SWP_NOZORDER);

	// вывод времени записи / воспроизведения / общая длительность аудио (внизу слева
	SetWindowPos(hWndTime, NULL, 70, H - 35, 195, 25, SWP_NOSIZE | SWP_NOZORDER);

	// вывод надписи "REC" внизу слева возле времени во время записи звука
	SetWindowPos(hWndRecSymbol, NULL, 5, H - 35, 60, 25, SWP_NOSIZE | SWP_NOZORDER);

	// перерисовываем фоны hGroupBox белой кистью, чтобы не было артефактов
...
LRESULT CALLBACK WindowFunc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		// шрифт для GroupBox и ComboBox
		hFontComboBox = CreateFont(
			18,                // Высота шрифта (в пунктах)
			0,                 // Ширина (0 = автоматически)
			0,                 // Угол наклона строки (0 = без наклона)
			0,                 // Угол наклона шрифта
			FW_MEDIUM,         // Толщина (FW_NORMAL, FW_BOLD и т. д.)
		...
		// шрифт для hList1 (мельче и тоньше)
		hFonthList1 = CreateFont(
			16,                // Высота шрифта (в пунктах)
			0,                 // Ширина (0 = автоматически)
			0,                 // Угол наклона строки (0 = без наклона)
			0,                 // Угол наклона шрифта
		...
		hBut1 = CreateWindow(L"button", L" Запись ", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP, \
			40, 215, 100, 30, hWnd, (HMENU)ID_REC, hInst, NULL);
		...
		hComboBox5 = CreateWindow(L"ComboBox", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | \
			CBS_DROPDOWNLIST, 8, 265, 324, 90, hWnd, (HMENU)ID_COMBOBOX5, hInst, NULL); // выбор устр. записи
		...
		hComboBox1 = CreateWindow(L"ComboBox", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | \
			CBS_AUTOHSCROLL | CBS_DROPDOWNLIST, 8, 295, 100, 100, hWnd, (HMENU)ID_COMBOBOX1, hInst, NULL);
		...
		hComboBox2 = CreateWindow(L"ComboBox", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | \
			CBS_DROPDOWNLIST, 92, 295, 117, 120, hWnd, (HMENU)ID_COMBOBOX2, hInst, NULL);
		...
		hComboBox3 = CreateWindow(L"ComboBox", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | \
			CBS_DROPDOWNLIST, 232, 295, 100, 70, hWnd, (HMENU)ID_COMBOBOX3, hInst, NULL);
		...
		hGroupBox2 = CreateWindow(L"button", L"Устройство записи:", WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
			5, 190, 330, 160, hWnd, (HMENU)ID_GROUPBOX2, hInst, NULL);
		SendMessage(hGroupBox2, WM_SETFONT, (WPARAM)hFontComboBox, TRUE);

		// раздел воспроизведения, КНОПКИ
		hBut3 = CreateWindow(L"button", L"Воспр.", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP, \
			10, 380, 90, 30, hWnd, (HMENU)ID_START_PLAY, hInst, NULL); // в секции воспроизведении
		...
		hButPPause = CreateWindow(L"button", L"Пауза", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP, \
			110, 380, 128, 30, hWnd, (HMENU)ID_PAUSE_PLAY, hInst, NULL); // в секции воспроизведении
		...
		hBut5 = CreateWindow(L"button", L"Стоп", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP, \
			230, 380, 92, 30, hWnd, (HMENU)ID_STOP_PLAY, hInst, NULL); // в секции воспроизведении
		SendMessage(hBut5, WM_SETFONT, (WPARAM)hFontButton, TRUE);
		// раздел воспроизведения, РАМКА
		hGroupBox3 = CreateWindow(L"button", L"Устройство воспроизведения:", WS_CHILD | WS_VISIBLE | BS_GROUPBOX,
			5, 340, 330, 65, hWnd, (HMENU)ID_GROUPBOX3, hInst, NULL);
		...
		hList1 = CreateWindow(L"listbox", NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | LBS_NOSEL,
			340, 347, 350, 155, hWnd, (HMENU)ID_LIST1, hInst, NULL);
		SendMessage(hList1, WM_SETFONT, (WPARAM)hFonthList1, TRUE);
		//DeleteObject(hFont); // нельзя, должен жить до конца жизни hList1
	
		// добавляем эффект объёмности рамок для списка hList1
		...
		hButCutSound = CreateWindow(L"button", L"_Обрезать звук_", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON | WS_TABSTOP,
			wW - 115, 200, 155, 30, hWnd, (HMENU)ID_SOUND_CUT, hInst, NULL);
		SendMessage(hButCutSound, WM_SETFONT, (WPARAM)hFontButton, TRUE);

		// вывод длительности текущей записи звука, текущего времени воспр. или общая длина аудиоданных
		hWndTime = CreateWindow(L"STATIC", L" 00:00:00 / 00:00:00", WS_CHILD | WS_VISIBLE | SS_CENTER /* | WS_BORDER*/,
			70, 450, 195, 25, hWnd, (HMENU)ID_STATIC_SOUND_TIME, hInst, NULL);
		SendMessage(hWndTime, WM_SETFONT, (WPARAM)hFontButton, TRUE);

		// выводим надпись "REC" внизу слева возле времени во время записи звука
		hWndRecSymbol = CreateWindow(L"STATIC", L"REC", WS_CHILD | WS_VISIBLE | SS_ICON | SS_CENTERIMAGE /* | WS_BORDER*/,
			5, 450, 60, 25, hWnd, (HMENU)ID_STATIC_REC_SYMBOL, hInst, NULL);
		...
		// скрываем надпись "REC" пока не начали запись
		ShowWindow(hWndRecSymbol, SW_HIDE);

		// вначале кнопки все кроме записи неактивны
		...
		// добавляем доступные данные о глубине звука и частоте его дискретизации
		...
		if (waveInDevLength > 0 && waveInDevLength < 50)
		{
	...
	case WM_SIZE:
	{
		...
		// для OpenGL отсчёт координат в окне с левого нижнего угла
		glViewportX = 5;
		glViewportY = 325;
		glViewportWidth = wW - 10;
		glViewportHeight = wH - 330;
		...
		RedrawWindowElements(LOWORD(lParam), HIWORD(lParam));
	...
	case WM_GETMINMAXINFO:
	{
		...
		// задаём минимально допустимый допустимый размер главного окна
		POINT ptMin = {715, 480};
